...
| 16 | The comparable field (Gate A format finding) | **`card_version: 2` adds `comparable` — the "X but Y" positioning line.** From mk's own field list for the card ("it's like pokemon go but with much more variety and meme potential" — the one sentence mk reaches for when pitching, and the format had no slot for it). Versioning per amendment A5: a bump must never invalidate the just-ratified card, so v1 cards keep five slots forever and the sixth arrives only where a card declares version 2. Governed by R1, exempt from R2 (a comparison to another product is external by nature). Encoded in card-check `7e8c627`; uncrancher is the first v2 card. |
| 17 | Finding 1: rules do not switch off at ratification | **R1/R2 govern `confirmed` fields; the escape is `origin: mk`, that literal only.** Mk's pick, 2026-08-17. Before this, a field written straight to `confirmed` bypassed citation and scope silently — the rubber stamp arriving one state later. Strict R1/R2-on-confirmed was rejected because it would have invalidated the pilot card's mk-originated success metric (the A5 trap through a rule instead of a version); the escape turns "originated by mk, not drafted from repo text" from prose into a machine-readable claim. Drafted fields never get the escape. Encoded in card-check `9e76f92` with tests that fail on the pre-ruling checker. Measured estate impact: palettice flipped invalid (cuj confirmed, uncited) — the predicted rubber stamp, caught on day one; left for first touch per ruling 12. |
