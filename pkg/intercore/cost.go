...
// CostBaseline queries ic cost baseline for the default 30-day window.
func (c *Client) CostBaseline(ctx context.Context) (*CostBaseline, error) {
	data, err := c.execJSON(ctx, "cost", "baseline", "--days=30")
	if err != nil {
		return nil, err
	}
	result, err := unmarshal[CostBaseline](data)
	if err != nil {
		return nil, err
	}
	return &result, nil
