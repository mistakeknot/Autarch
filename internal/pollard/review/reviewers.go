// Package review provides multi-agent research quality validation for Pollard.
// Following the Compound Engineering pattern, multiple specialized reviewers
// analyze insights in parallel to catch issues a single reviewer might miss.
package review
...
// InsightReviewer validates research quality from a specific perspective.
type InsightReviewer interface {
	// Review analyzes an insight and returns findings.
	Review(ctx context.Context, insight *insights.Insight) (*ReviewResult, error)
...
type Issue struct {
	...
	// Location points to where in the insight the issue was found.
	Location string
}
...
const (
	SeverityError   IssueSeverity = "error"   // Must fix before using
...
type CombinedReview struct {
	// InsightID identifies the reviewed insight.
	InsightID string
	...
	// Passed indicates if the insight meets quality thresholds.
	Passed bool
}
...
func RunParallelReview(ctx context.Context, insight *insights.Insight, reviewers []InsightReviewer) (*CombinedReview, error) {
	...
	for _, reviewer := range reviewers {
		wg.Add(1)
		go func(r InsightReviewer) {
			defer wg.Done()

			result, err := r.Review(ctx, insight)
			if err != nil {
				errorsCh <- err
				return
			}
	...
	combined.Passed = combined.OverallScore >= PassThreshold && combined.Errors == 0

	return combined, nil
}

// DefaultReviewers returns the standard set of insight reviewers.
func DefaultReviewers() []InsightReviewer {
	return []InsightReviewer{
		NewSourceCredibilityReviewer(),
		NewRelevanceReviewer(),
		NewContradictionDetector(),
	}
}
