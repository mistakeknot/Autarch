...
CREATE TABLE IF NOT EXISTS launch_instance (
  ...
  -- Read from the same ps run as the liveness probe, because the process
  -- table is the only place it exists and it stops existing the moment the
  -- process does. A row whose process was never successfully probed keeps
  -- NULL: not observed is not the same fact as no parent.
  ppid              INTEGER,
  -- Set only when that ppid is the pid of another instance this registry
  -- holds open in the same host and pid domain -- a join against a record we
  -- have, never a guess. The dispatch shim in between means it is not always
  -- the conversation's parent; any richer inference lands in
  -- conversation_lineage with its own basis.
...
CREATE TABLE IF NOT EXISTS pane_binding (
  ...
  -- What the session record CLAIMED, kept beside what the server was OBSERVED
  -- to say. Verification used to overwrite window_id and session_name_seen
  -- with the observation, which destroyed the only evidence that the two ever
  -- disagreed -- and they do disagree: measured 2026-09-19, a record claimed
  -- session "tmux-organizer" for a pane the live server called
  -- "iterm[autarch - e4bedaf5...". A disagreement is a fact about the estate,
  -- not a defect in the claim, and a verified row that silently adopts the
  -- server's answer can no longer be asked whether it was ever wrong.
  claimed_window_id    TEXT NOT NULL DEFAULT '',
  claimed_session_name TEXT NOT NULL DEFAULT '',
  binding_basis     TEXT NOT NULL
                      CHECK (binding_basis IN ('tmux_inventory','session_file_claim')),
  -- A second instrument agreeing, recorded because the first one cannot tell
  -- servers apart. Window ids are per-server counters exactly as pane ids
  -- are: %0 in @0 exists on every tmux server, so a claim from an unswept
  -- server can pass the window gate against this server's %0. Measuring that
  -- the gate admits true claims -- 11 of 11 on 2026-09-19 -- says nothing
  -- about the rate at which it admits false ones, which an estate with one
  -- socket cannot sample at all.
  --
  -- 'parent_pid' means the process table independently put this agent's
  -- parent at this pane's root process, and a pid is host-unique, so that is
  -- a different instrument reaching the same answer. NULL means no
  -- corroboration was available, not that one failed: a dispatched child's
  -- parent is legitimately the shim, never the pane. It is never a gate for
  -- that reason -- but a CONTRADICTION is, because a parent that is some
  -- OTHER pane's root process on the swept server is positive evidence.
  corroborated_by   TEXT CHECK (corroborated_by IS NULL OR corroborated_by IN ('parent_pid')),
  ...
  pane_key          TEXT GENERATED ALWAYS AS (
                      ...
                      END) STORED,
  -- When a complete sweep of this binding's own server last listed its pane.
  -- The exact analogue of launch_instance.last_alive_ms, and needed for the
  -- same reason: without it an open binding only means "not known to have
  -- ended", so one confirmed present a moment ago and one last seen on a
  -- server that has since restarted render identically. A claim has none,
  ...
  CHECK (binding_basis <> 'tmux_inventory' OR (socket IS NOT NULL AND server_pid IS NOT NULL
         AND server_started IS NOT NULL AND pane_pid IS NOT NULL)),
  CHECK (observed_to_ms IS NULL OR (end_basis IS NOT NULL AND end_event_id IS NOT NULL)),
  -- A presence claim cites the sweep that made it, exactly as a closure cites
  -- the event that closed it. Presence with no event behind it is a timestamp
  -- asserting something nothing observed.
  CHECK (last_present_ms IS NULL OR last_present_event_id IS NOT NULL),
  -- Only a verified binding can be present: presence means a complete sweep of
...
CREATE TABLE IF NOT EXISTS projection_state (
  ...
  -- Refusals need somewhere that outlives the pass that made them. They were
  -- reported only through source.last_error, which the NEXT successful pass
  -- overwrites -- and one sweep projects twice, so a refusal from the first pass
  -- survived milliseconds. A refusal is the projector declining to act on an
  -- event; if the only lasting record is a log line, then for every reader of
  -- this database it did not happen.
  refusal_count            INTEGER NOT NULL DEFAULT 0,
  last_refusal_event_id    INTEGER,
  last_refusal             TEXT
);
