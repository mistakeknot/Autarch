...
// ColdwineChatHandler implements ChatHandler for the Coldwine dashboard.
...
// NewColdwineChatHandler creates a Coldwine chat handler.
func NewColdwineChatHandler() *ColdwineChatHandler {
	cwd, _ := os.Getwd()
	return &ColdwineChatHandler{cwd: cwd}
}

// HandleMessage sends the user's message to Claude with Coldwine context.
func (h *ColdwineChatHandler) HandleMessage(ctx context.Context, userMsg string) (<-chan pkgtui.StreamMsg, error) {
...
func (h *ColdwineChatHandler) SetContinue(cont bool, sessionID string) {
	h.mu.Lock()
