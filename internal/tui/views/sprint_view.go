...
func (v *SprintView) StartSprintWithExploration(userInput string, artifacts *scan.Artifacts, exploration map[string]any, sessionID string) tea.Cmd {
	return func() tea.Msg {
		...
		// Cache exploration result and session ID for phase transitions
		if exploration != nil {
			v.orch.SetExplorationResult(exploration, sessionID)
		}
		state, _ := v.orch.State()
		return tui.SprintDraftUpdatedMsg{
			Phase:   state.Phase.String(),
...
func (v *SprintView) Update(msg tea.Msg) (pkgtui.View, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case pkgtui.FocusDocument:
			...
			// Use ctrl+p/n (Emacs-style prev/next) to avoid tmux/terminal conflicts
			switch msg.String() {
			case "up", "ctrl+p":
				v.docPanel.ScrollUp()
				return v, nil
